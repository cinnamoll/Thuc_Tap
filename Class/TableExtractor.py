...
class TableExtractor:
    def __init__(self):
        self.dpi = 400
        self.RE_FINANCIAL = re.compile(r"^\(?\d[\d,. ]{4,}\d\)?$")
        self.RE_CODE = re.compile(r"^\d{2,3}$")
        self.RE_NOTES_NUM = re.compile(r"^\d{1,2}$")
        self.RE_SHORT_NUM = re.compile(r"^[(]?\d{1,3}[a-zA-Z]?[)]?$")
        self.FOOTER_KW = ["chief accountant", "general director", "prepared by", "director"]
    ...
    def group_into_lines_ocr(self, tagged_words: list[tuple]) -> list[list[Word]]:
        buckets: dict = defaultdict(list)
        for word, key in tagged_words:
            buckets[key].append(word)
        sorted_keys = sorted(buckets.keys(), key=lambda k: sum(w.top for w in buckets[k]) / len(buckets[k]))
        lines = []
    ...
    def detect_data_start(self, lines: list[list[Word]]) -> int:
        for idx, line in enumerate(lines):
            has_fin = any(self.is_financial_number(w.text) for w in line)
            has_code = any(self.RE_CODE.match(w.text) for w in line)
            if has_fin and has_code:
                return max(0, idx - 1)
        return 0

    def infer_bands_from_data(self, data_lines: list[list[Word]], page_width: float) -> list[tuple[str, float, float]]:
        fin_words = []
        short_num_words = []
        
        for line in data_lines:
            fin_on_line = [w for w in line if self.is_financial_number(w.text)]
            if not fin_on_line: 
                continue
            min_fin_x0 = min(w.x0 for w in fin_on_line)
            fin_words.extend(fin_on_line)
            for w in line:
                if self.is_financial_number(w.text): 
                    continue
                if self.RE_SHORT_NUM.match(w.text.strip()) and w.x1 < min_fin_x0:
        ...
        notes_band = None
        if short_num_words:
            short_clusters = self.cluster_positions([w.x1 for w in short_num_words], gap_pt)
    ...
    def words_to_rows(self, words: list[Word], page_width: float) -> list[Row]:
        lines = self.group_into_lines(words)
        start_idx = self.detect_data_start(lines)
        data_lines = lines[start_idx:]
        
        bands = self.infer_bands_from_data(data_lines, page_width)
        if not bands:
            return []
            ...
            for w in line:
                ...
                for name, start, end in abs_bands:
                    ...
                    if start <= a <= end:
                        ...
                        break
                if assigned is None:
                    valid_bands = [b for b in abs_bands if not (b[0] == "items" and is_fin)]
                    if not valid_bands:
        ...
        for row in raw_rows:
            ...
            if "Items" in t or "items" in t or "Code" in t or "code" in t:  
                continue
            if (not t) or any(kw in t for kw in self.FOOTER_KW): 
                break
    ...
    def words_to_rows_with_lines(self, lines: list[list[Word]], all_words: list[Word], page_width: float) -> list[Row]:
        start_idx = self.detect_data_start(lines)
        data_lines = lines[start_idx:]
        ...
        for row in raw_rows:
            t = row.cells.get("items", "").lower()
            if "items" in t or "code" in t:
                continue
            if (not t and not pending_items) or any(kw in t for kw in self.FOOTER_KW):
                break
            ...
            if not has_code and not has_numbers and items_text:
    ...
    def clean_row(self, row: Row) -> dict:
        ...
        for k, v in row.cells.items():
            if k not in ("items", "code", "notes"):
                result[k] = self.parse_number(v)
        return result

    def standardize_row(self, row_dict: dict, report_type: str) -> dict:
        new_row = {
            "Prefix": row_dict.get("prefix", ""),
            "Items": row_dict.get("chi_tieu", ""),
            "Code": row_dict.get("ma_so", ""),
            "Notes": row_dict.get("notes", "")
        }
        if report_type == "BS":
            new_row["period_current"] = row_dict.get("period_current")
            new_row["period_prior"] = row_dict.get("period_prior")
        elif report_type == "PL":
            new_row["period_current"] = row_dict.get("period_current")
            new_row["period_prior"] = row_dict.get("period_prior")
            new_row["accum_current"] = row_dict.get("accum_current")
            new_row["accum_prior"] = row_dict.get("accum_prior")
        elif report_type == "CF":
            new_row["accum_current"] = row_dict.get("accum_current") if "accum_current" in row_dict else row_dict.get("period_current")
            new_row["accum_prior"] = row_dict.get("accum_prior") if "accum_prior" in row_dict else row_dict.get("period_prior")
        return new_row
    ...
    def extract_table(self, file_path: str, page_start: int, page_end: int, report_type: str) -> list[dict]:
        ...
        with pdfplumber.open(file_path) as pdf:
            ...
            for i, page_num in enumerate(range(page_start, page_end + 1)):
                ...
                has_fin = False
                for line in lines:
                    if any(self.is_financial_number(w.text) for w in line):
                        has_fin = True
                        break

                ocr_lines = None
                if not has_fin:
                    ocr_data = pytesseract.image_to_data(imgs[i], output_type=pytesseract.Output.DICT)
                    img_w, img_h = imgs[i].size
                    ...
                    for j in range(len(ocr_data["text"])):
                        ...
                        tagged_words.append((Word(text, x, x + w_px, y, y + h_px), line_key))
                    words = [tw[0] for tw in tagged_words]
                    ocr_lines = self.group_into_lines_ocr(tagged_words)

                if ocr_lines is not None:
                    rows = self.words_to_rows_with_lines(ocr_lines, words, w_pt)
                else:
                    rows = self.words_to_rows(words, w_pt)
                ...
                for r in rows:
                    ...
                    if not any(v is not None for k, v in c.items() if k not in ('prefix', 'chi_tieu', 'ma_so', 'notes')):
                        pass
                    all_rows.append(self.standardize_row(c, report_type))
        return all_rows