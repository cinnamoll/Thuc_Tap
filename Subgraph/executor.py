...
def binning_standardizing_tool(action: EngineeringAction, output_path:str) -> str:
    """
    Apply standard scale normalization to continuous data columns.

    Args:
        file_path (str): path to the dataset file
        column (str): name of the continuous column to analyze
    ...
    if action.actionType == BinningType.STANDARD:
        ...
        raise ValueError(f"Unsupported binning type: {action.actionType}")
...
def executor_node(state: AgentState) -> dict:
    ...
    pending = state.get(f'pending_{action_type}', []) if action_type in ['cleaning', 'engineering', 'insight'] else []
    if not pending:
        return {"action_res": "No pending action to execute", "skip_confirm": True, "fallback_used": False, "action_status": True}
    action = pending[-1]
    ...
    try:
        if isinstance(action, CleaningAction):
            ...
            if isinstance(action.actionType, BinningType):
...
def review_execution_node(state: AgentState):
    ...
    pending = state.get(f'pending_{action_type}', []) if action_type in ['cleaning', 'engineering', 'insight'] else []
    if not pending:
        return {"completed_actions": [], "retry_count": 0, "review_decision": "accepted"}
    action = pending[-1]
